...
class M_Admin extends CI_Model
{
  ...
  public function is_book_available($buku_id)
    {
        $this->db->where('id_buku', $buku_id);
        $buku = $this->db->get('tbl_buku')->row();

        if ($buku) {
            $dipinjam = $this->db->where('buku_id', $buku_id)
                ->where('status', 'Dipinjam')
                ->count_all_results('tbl_pinjam');
            return $buku->jml > $dipinjam;
        }
